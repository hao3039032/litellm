...
def _get_openai_compatible_provider_info(
    ...
    # publicai is now handled by JSON config (see litellm/llms/openai_like/providers.json)
        ...
        ) = litellm.OpenCodeZenChatConfig()._get_openai_compatible_provider_info(api_base, api_key)  # pyright: ignore[reportPrivateUsage]  # sibling providers call the same private resolver
    elif custom_llm_provider == "opencode_go":
        (
            api_base,  # rebind-ok: resolution chain returns these params
            dynamic_api_key,  # rebind-ok: resolution chain returns these params
        ) = litellm.OpenCodeGoChatConfig()._get_openai_compatible_provider_info(api_base, api_key)  # pyright: ignore[reportPrivateUsage]  # sibling providers call the same private resolver
