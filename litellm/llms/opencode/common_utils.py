...
def with_opencode_session_header(
    ...
    proxy_request: Final = litellm_params.get("proxy_server_request")
    inbound_headers: Final = proxy_request.get("headers") if isinstance(proxy_request, Mapping) else None
    inbound_session: Final = (
        next(
            (
                value
                for name, value in inbound_headers.items()
                if isinstance(name, str)
                and name.lower() == OPENCODE_SESSION_HEADER
                and isinstance(value, str)
                and value
            ),
            None,
        )
        if isinstance(inbound_headers, Mapping)
        else None
    )
    session_id: Final = (
        None if has_opencode_session_header(headers) else inbound_session or resolve_opencode_session_id(litellm_params)
    )
    stamped: Final = () if session_id is None else ((OPENCODE_SESSION_HEADER, session_id),)
    return dict(chain(headers.items(), stamped))  # mutable-ok: HTTP handler mutates the returned headers
...
def opencode_endpoint_for_model(provider: str, model: str) -> str:
    ...
    entry: Final = litellm.model_cost.get(f"{provider}/{lowered}") or litellm.model_cost.get(lowered)
