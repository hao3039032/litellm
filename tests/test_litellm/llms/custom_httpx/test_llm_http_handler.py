...
@pytest.mark.asyncio
async def test_async_custom_stream_wrapper_uses_model_proxy():
    handler = BaseLLMHTTPHandler()
    config = Mock()
    config.has_custom_stream_wrapper = True
    config.get_async_custom_stream_wrapper = AsyncMock(return_value=Mock())
    ...
    with patch(
        ...
        await handler.acompletion_stream_function(
    ...
    assert config.get_async_custom_stream_wrapper.call_args.kwargs["client"] is client
