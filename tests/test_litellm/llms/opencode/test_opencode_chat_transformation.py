    ...
    def test_tiered_pricing_applies_above_the_context_threshold(self, local_model_cost_map):
        ...
        def cost_for(prompt_tokens):
            response = ModelResponse(
                ...
                usage=Usage(prompt_tokens=prompt_tokens, completion_tokens=1_000, total_tokens=prompt_tokens + 1_000),
            )
...
class TestMessagesCompletionWiring:
    ...
    @pytest.mark.parametrize(
        "model, expected_url",
        [
            ("MiniMax-M3", f"{GO_API_BASE}/messages"),
            ("MiniMax-M2.7", f"{GO_API_BASE}/messages"),
        ],
    )
    def test_mixed_case_model_group_name_still_routes_to_messages(self, local_model_cost_map, model, expected_url):
        """Proxy deployments expose model_group names like ``MiniMax-M3`` while the cost
        map keys are lowercase; the endpoint lookup must not miss on case alone."""
        client = RecordingAnthropicHTTPHandler(response_model="minimax-m3")
        litellm.completion(
            model=model,
            custom_llm_provider="opencode_go",
            messages=[{"role": "user", "content": "hello"}],
            api_key="fake-key",
            litellm_session_id="session-43",
            client=client,
        )

        sent = client.requests[0]
        assert sent["url"] == expected_url
        assert sent["headers"][OPENCODE_SESSION_HEADER] == "session-43"


@pytest.mark.parametrize("surface", ["chat", "messages", "responses"])
@pytest.mark.parametrize("session_source", ["explicit", "proxy", "session", "missing"])
@pytest.mark.asyncio
async def test_minimax_api_surfaces_preserve_session_and_tools(local_model_cost_map, surface, session_source):
    ...
    def respond(request):
        ...
        return httpx.Response(
            ...
            json={
                ...
                "role": "assistant",
                "model": "minimax-m3",
                "content": [{"type": "tool_use", "id": "tool_123", "name": "lookup", "input": {"query": "test"}}],
    ...
    session_kwargs = {
        "explicit": {
            "extra_headers": {"X-OpenCode-Session": "explicit-session"},
            "litellm_session_id": "other-session",
        },
        "proxy": {
            "proxy_server_request": {"headers": {"X-OpenCode-Session": "proxy-session"}},
            "litellm_session_id": "other-session",
        },
        "session": {"litellm_session_id": "stable-session"},
        "missing": {},
    }[session_source]
    async with httpx.AsyncClient(transport=httpx.MockTransport(respond)) as transport:
        ...
        common = {"model": "opencode_go/minimax-m3", "api_key": "fake-key", "client": client, **session_kwargs}
        function = {
            "name": "lookup",
            ...
            if surface == "chat":
                ...
                response = await litellm.anthropic.messages.acreate(
                    **common,
                    ...
                    messages=[{"role": "user", "content": turn}],
                    tools=[
                        {"name": "lookup", "description": "Look up a query", "input_schema": function["parameters"]}
                    ],
                )
                assert response["content"][0]["name"] == "lookup"
            else:
                response = await litellm.aresponses(**common, input=turn, tools=[{"type": "function", **function}])
                assert any(item.type == "function_call" and item.name == "lookup" for item in response.output)

    assert len(requests) == 2
    for request in requests:
        assert str(request.url) == GO_API_BASE + "/messages"
        assert request.headers["x-api-key"] == "fake-key"
        assert "authorization" not in request.headers
        assert request.headers[OPENCODE_SESSION_HEADER]
        assert json.loads(request.content)["tools"][0]["name"] == "lookup"
    sent_sessions = [r.headers[OPENCODE_SESSION_HEADER] for r in requests]
    if session_source == "missing":
        assert sent_sessions[0] != sent_sessions[1]
    else:
        expected = {"explicit": "explicit-session", "proxy": "proxy-session", "session": "stable-session"}[
            session_source
        ]
        assert sent_sessions == [expected, expected]


@pytest.mark.parametrize("surface", ["chat", "messages", "responses"])
@pytest.mark.asyncio
async def test_minimax_streaming_surfaces(local_model_cost_map, surface):
    from litellm.llms.custom_httpx.http_handler import AsyncHTTPHandler
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(respond)) as transport:
        ...
        common = {
            ...
            "api_key": "fake-key",
            "client": client,
            "litellm_session_id": "stream-session",
            "stream": True,
        }
        if surface == "chat":
            response = await litellm.acompletion(**common, messages=[{"role": "user", "content": "hello"}])
        elif surface == "messages":
            response = await litellm.anthropic.messages.acreate(
                **common, max_tokens=100, messages=[{"role": "user", "content": "hello"}]
            )
        else:
            response = await litellm.aresponses(**common, input="hello")
        chunks = [chunk async for chunk in response]
        assert chunks
        assert "OK" in str(chunks)
    assert len(requests) == 1
    assert str(requests[0].url) == GO_API_BASE + "/messages"
    assert requests[0].headers[OPENCODE_SESSION_HEADER] == "stream-session"
    assert json.loads(requests[0].content)["stream"] is True


@pytest.mark.asyncio
async def test_router_fallback_preserves_opencode_session(local_model_cost_map):
    from litellm.llms.custom_httpx.http_handler import AsyncHTTPHandler
    ...
    def respond(request):
        requests.append(request)
        if request.headers["x-api-key"] == "primary-key":
            return httpx.Response(429, json={"error": {"type": "rate_limit_error", "message": "try backup"}})
        return httpx.Response(
            200,
            json={
                "id": "msg_backup",
    ...
    router = litellm.Router(
        model_list=[
            {"model_name": "primary", "litellm_params": {"model": "opencode_go/minimax-m3", "api_key": "primary-key"}},
            {"model_name": "backup", "litellm_params": {"model": "opencode_go/minimax-m3", "api_key": "backup-key"}},
        ],
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(respond)) as transport:
        ...
        response = await router.acompletion(
            model="primary",
            messages=[{"role": "user", "content": "hello"}],
            litellm_session_id="fallback-session",
            client=client,
        )
        assert response.choices[0].message.content == "OK"
    assert len(requests) >= 2
    assert requests[0].headers["x-api-key"] == "primary-key"
    assert requests[-1].headers["x-api-key"] == "backup-key"
    assert {r.headers[OPENCODE_SESSION_HEADER] for r in requests} == {"fallback-session"}
