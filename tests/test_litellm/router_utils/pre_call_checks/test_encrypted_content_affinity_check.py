...
def test_boundary_key_rejects_non_dict_like_inputs():
    ...
    valid boundary. An empty api_base with an api_key IS a valid boundary
    (key-only deployments: the key alone identifies the org).
    """
    from litellm.router_utils.pre_call_checks.encrypted_content_affinity_check import (
        EncryptedContentAffinityCheck,
    )
...
def credential_store():
    """
    Populate ``litellm.credential_list`` with two credentials mirroring real
    deployments: one api_key-only (first-party org), one api_key+api_base.
    Restores the prior list afterwards so other tests are unaffected.
    """
    ...
    litellm.credential_list = [  # test-quality-ok: fixture-populated credential store; restored on teardown below
        CredentialItem(
            ...
            credential_info={},
            credential_values={"api_key": "key-jmo"},
        ),
        CredentialItem(
            credential_name="creds-opencode",
            credential_info={},
            credential_values={"api_key": "key-oc", "api_base": "https://oc.example.com"},
        ),
    ]
...
def test_boundary_key_resolves_credential_name(credential_store):
    """
    Deployments referencing credentials via ``litellm_credential_name`` carry
    no inline api_base/api_key at router level; the boundary must resolve
    through CredentialAccessor or these deployments never match any peer
    and every model-group switch fails with a spurious 503.
    """
    from litellm.router_utils.pre_call_checks.encrypted_content_affinity_check import (
        EncryptedContentAffinityCheck,
    )

    key_only = EncryptedContentAffinityCheck._encryption_boundary_key({"litellm_credential_name": "creds-jmo"})
    assert key_only == ("", "key-jmo")

    with_base = EncryptedContentAffinityCheck._encryption_boundary_key({"litellm_credential_name": "creds-opencode"})
    assert with_base == ("https://oc.example.com", "key-oc")


def test_boundary_key_inline_values_win_over_credential(credential_store):
    from litellm.router_utils.pre_call_checks.encrypted_content_affinity_check import (
        EncryptedContentAffinityCheck,
    )

    assert EncryptedContentAffinityCheck._encryption_boundary_key(
        {"litellm_credential_name": "creds-jmo", "api_key": "inline-key"}
    ) == ("", "inline-key")


def test_boundary_key_unknown_credential_name_falls_back_to_inline(credential_store):
    from litellm.router_utils.pre_call_checks.encrypted_content_affinity_check import (
        EncryptedContentAffinityCheck,
    )

    assert EncryptedContentAffinityCheck._encryption_boundary_key({"litellm_credential_name": "does-not-exist"}) is None


def test_boundary_key_matches_credential_backed_with_inline_peer(credential_store):
    """
    A credential-backed originating deployment and an inline-configured peer
    holding the same underlying key must land on the same boundary.
    """
    from litellm.router_utils.pre_call_checks.encrypted_content_affinity_check import (
        EncryptedContentAffinityCheck,
    )
    ...
    assert EncryptedContentAffinityCheck._encryption_boundary_key(
        {"litellm_credential_name": "creds-jmo"}
    ) == EncryptedContentAffinityCheck._encryption_boundary_key({"api_key": "key-jmo"})


@pytest.mark.asyncio
async def test_affinity_pins_to_same_credential_peer_on_model_group_switch(credential_store):
    """
    Production topology (luna d3a475d7 -> sol 11602244, both backed by the
    same api_key-only credential): the model-group switch must pin to the
    sol deployment instead of raising a spurious 503.
    """
    first_resp = _build_mock_response(
        output_items=[
            {
    ...
    try:
        with patch(
            ...
            r1 = await router.aresponses(model="gpt-5.6-luna", input="hi")
        encoded_id = _extract_encoded_item_id(r1)
        assert encoded_id.startswith("encitem_")
        ...
        assert r2._hidden_params["model_id"] == "sol-jmo"
    finally:
        router.discard()
...
# ---------------------------------------------------------------------------
# drop_stale_encrypted_content: drop stale encrypted items instead of 503
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_drop_stale_encrypted_content_routes_normally_on_cross_org_switch():
    """
    Cross-org model switch (OpenCodeGo luna -> JmoGameSG sol) with
    ``drop_stale_encrypted_content=True``: the encrypted reasoning items cannot
    be honored by any sol deployment, so they are dropped from the input, the
    visible history is kept, and the request routes normally instead of
    failing with 503.
    """
    first_resp = _build_mock_response(
        output_items=[
        ...
        model_list=[
            {
                ...
                "litellm_params": {
                    "model": "openai/gpt-5.6",
                    "api_key": "key-opencodego",
                },
                "model_info": {"id": "luna-opencodego"},
            },
            {
                "model_name": "gpt-5.6-sol",
                "litellm_params": {
                    "model": "openai/gpt-5.6-sol",
                    "api_key": "key-jmogamesg",
                },
                "model_info": {"id": "sol-jmogamesg"},
            },
        ],
        optional_pre_call_checks=["encrypted_content_affinity"],
    ...
    try:
        ...
        assert r2._hidden_params["model_id"] == "sol-jmogamesg"
        assert len(captured_inputs) == 1
        forwarded_input = captured_inputs[0]
        assert forwarded_input is not None
        assert len(forwarded_input) == 1
        assert forwarded_input[0]["id"] == "msg_visible_history"
    finally:
        router.discard()


@pytest.mark.asyncio
async def test_drop_stale_encrypted_content_off_by_default_keeps_503():
    """
    Without the flag, the same cross-org switch must keep failing fast with
    503 — the drop is opt-in.
    """
    from litellm.exceptions import ServiceUnavailableError
    ...
    try:
        encoded_id = ResponsesAPIRequestUtils._build_encrypted_item_id("luna-opencodego", "rs_test")
...
@pytest.mark.asyncio
async def test_drop_stale_encrypted_content_drops_wrapped_content_without_id():
    """
    Codex-style items (no item ID, model_id wrapped inside encrypted_content)
    must also be dropped by the flag.
    """
    router = litellm.Router(
        model_list=[
            {
    ...
    check = next(cb for cb in (router.optional_callbacks or []) if type(cb).__name__ == "EncryptedContentAffinityCheck")
    wrapped = ResponsesAPIRequestUtils._wrap_encrypted_content_with_model_id(
        "gAAAAAB_original", "some-removed-deployment"
    )
    request_kwargs = {
        "input": [
            {"type": "message", "role": "user", "content": "hi"},
            {"type": "reasoning", "encrypted_content": wrapped},
        ],
        "litellm_metadata": {},
    }
    ...
    filtered = await check.async_filter_deployments(
        ...
        healthy_deployments=[
            {
                "model_info": {"id": "sol-jmogamesg"},
                "litellm_params": {"model": "openai/gpt-5.6-sol", "api_key": "key-jmogamesg"},
            }
        ],
        messages=None,
        request_kwargs=request_kwargs,
    ...
    assert request_kwargs["input"][0]["type"] == "message"
    router.discard()


@pytest.mark.asyncio
async def test_drop_stale_encrypted_content_keeps_non_encoded_items():
    """
    Items with plain IDs / plain encrypted_content (not litellm-encoded) are
    not touched by the drop; only encoded markers are stale by definition.
    """
    router = litellm.Router(
        model_list=[
            {
    ...
    check = next(cb for cb in (router.optional_callbacks or []) if type(cb).__name__ == "EncryptedContentAffinityCheck")
    request_input = [
        {"type": "reasoning", "id": "rs_plain", "encrypted_content": "gAAAAAB_plain"},
        {"type": "function_call", "id": "fc_1", "name": "get_weather", "arguments": "{}"},
    ]

    dropped = check._drop_stale_encrypted_items(request_input)

    assert dropped == request_input
    router.discard()
