...
def test_model_proxy_repr_hides_credentials():
    from litellm.types.router import LiteLLM_Params

    params = LiteLLM_Params(
        model="openai/gpt-5",
        proxy="http://proxy-user:proxy-password@proxy.example.com:8080",
    )

    assert "proxy-user" not in repr(params)
    assert "proxy-password" not in repr(params)


def test_router_print_deployment_masks_model_proxy_credentials():
    router = litellm.Router(
        model_list=[
            {
                "model_name": "gpt-5-proxied",
    ...
    printed = router.print_deployment(router.model_list[0])

    assert printed["litellm_params"]["proxy"] == "http://***:***@proxy.example.com:8080"
